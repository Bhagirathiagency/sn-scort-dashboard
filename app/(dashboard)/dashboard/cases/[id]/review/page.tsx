import Link from "next/link";
import { notFound, redirect } from "next/navigation";
import { getSessionUser } from "@/lib/supabase/server";
import { getCaseDetail } from "@/lib/cases/service";
import { listCaseReviews } from "@/lib/cases/reviews";
import { STATUS_LABEL } from "@/lib/cases/labels";
import type { ReviewStage } from "@/lib/cases/types";
import { ReviewForm } from "./review-form";

const DECISION_LABEL: Record<string, string> = {
  approved: "Approved",
  returned: "Returned for rework",
  comment: "Comment",
};

export default async function MedicalReviewPage({ params }: { params: { id: string } }) {
  const user = await getSessionUser();
  if (!user || !user.organizationId) redirect("/login");

  const caseDetail = await getCaseDetail(user.organizationId, params.id);
  if (!caseDetail) notFound();

  const reviews = await listCaseReviews(user.organizationId, params.id);
  const isReviewable = caseDetail.status === "medical_review" || caseDetail.status === "qc";
  const stage: ReviewStage = caseDetail.status === "qc" ? "qc" : "medical_review";

  return (
    <div className="mx-auto max-w-4xl">
      <header className="mb-6">
        <Link href={`/dashboard/cases/${caseDetail.id}`} className="text-sm text-teal-600 hover:underline">
          &larr; {caseDetail.case_number}
        </Link>
        <div className="mt-1 flex items-center gap-3">
          <h1 className="text-2xl font-semibold text-navy-900">Medical Review</h1>
          <span className="pv-badge bg-teal-50 text-teal-600">{STATUS_LABEL[caseDetail.status]}</span>
          {caseDetail.is_serious && <span className="pv-badge bg-safe-red/10 text-safe-red">Serious</span>}
        </div>
      </header>

      <div className="grid grid-cols-2 gap-4">
        <section className="pv-card p-5">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-navy-600">Patient</h2>
          <dl className="space-y-1 text-sm">
            <Row label="Initials" value={caseDetail.patient?.initials} />
            <Row label="Age" value={caseDetail.patient?.age?.toString()} />
            <Row label="Sex" value={caseDetail.patient?.sex} />
          </dl>
        </section>

        <section className="pv-card p-5">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-navy-600">Reporter</h2>
          <dl className="space-y-1 text-sm">
            <Row label="Type" value={caseDetail.reporter?.reporter_type} />
            <Row
              label="HCP"
              value={caseDetail.reporter ? (caseDetail.reporter.is_healthcare_professional ? "Yes" : "No") : undefined}
            />
          </dl>
        </section>

        <section className="pv-card p-5">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-navy-600">
            Suspect / Concomitant Products
          </h2>
          <ul className="space-y-2 text-sm">
            {caseDetail.case_products.map((cp, i) => (
              <li key={i}>
                {cp.product?.product_name ?? "Unknown"}{" "}
                <span className="pv-badge bg-navy-50 text-navy-600">{cp.role}</span>
              </li>
            ))}
            {caseDetail.case_products.length === 0 && <p className="text-navy-600">None recorded.</p>}
          </ul>
        </section>

        <section className="pv-card p-5">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-navy-600">Adverse Events</h2>
          <ul className="space-y-2 text-sm">
            {caseDetail.case_events.map((ev, i) => (
              <li key={i}>
                {ev.verbatim_term} — MedDRA: {ev.meddra_term ?? "Not yet coded"}
              </li>
            ))}
            {caseDetail.case_events.length === 0 && <p className="text-navy-600">None recorded.</p>}
          </ul>
        </section>
      </div>

      <section className="pv-card mt-4 p-5">
        <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-navy-600">Narrative</h2>
        <p className="whitespace-pre-wrap text-sm text-navy-900">
          {caseDetail.narrative || "No narrative recorded yet."}
        </p>
      </section>

      <div className="mt-4">
        {isReviewable ? (
          <ReviewForm caseId={caseDetail.id} stage={stage} />
        ) : (
          <p className="pv-card p-5 text-sm text-navy-600">
            This case is currently &ldquo;{STATUS_LABEL[caseDetail.status]}&rdquo; — Medical Review and QC
            actions are only available while a case is in the Medical Review or QC stage.
          </p>
        )}
      </div>

      <section className="pv-card mt-4 p-5">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-navy-600">Review History</h2>
        <ul className="space-y-3">
          {reviews.map((r) => (
            <li key={r.id} className="border-b border-navy-100 pb-3 last:border-0 text-sm">
              <div className="flex items-center gap-2">
                <span className="pv-badge bg-navy-50 text-navy-600">
                  {r.stage === "medical_review" ? "Medical Review" : "QC"}
                </span>
                <span className="pv-badge bg-teal-50 text-teal-600">{DECISION_LABEL[r.decision]}</span>
                <span className="text-xs text-navy-600">
                  {r.reviewer?.full_name ?? r.reviewer?.email ?? "Unknown reviewer"} ·{" "}
                  {new Date(r.created_at).toLocaleString()}
                </span>
              </div>
              {r.comment && <p className="mt-1 text-navy-900">{r.comment}</p>}
            </li>
          ))}
          {reviews.length === 0 && <p className="text-sm text-navy-600">No reviews recorded yet.</p>}
        </ul>
      </section>
    </div>
  );
}

function Row({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div className="flex justify-between">
      <dt className="text-navy-600">{label}</dt>
      <dd className="font-medium text-navy-900">{value || "—"}</dd>
    </div>
  );
}
