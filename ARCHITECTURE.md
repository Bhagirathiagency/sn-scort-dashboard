...
- **Development infrastructure is now provisioned.** A dedicated Supabase project (`pv-plus-dev`, ref `bfdrqujjeenkmgmvphax`) has all Phase 0 migrations applied, RLS enabled, and a clean security-advisor pass (two follow-up migrations fixed a mutable-`search_path` and anon-execute finding on the RLS helper functions). The app builds successfully against it. A separate, unrelated Supabase project already existed on the same account (an active lead-gen tool with live prospect data) — PV+ was deliberately isolated into its own new project rather than sharing that one, preserving the tenant/environment-separation principle even at the infrastructure level. Hosting (Vercel or equivalent) for a publicly reachable deployment is still not provisioned. Test/Validation/Production Supabase projects (§32) are also not yet provisioned — each is a deliberate later step, not implied by Development existing.
...
- **Demo data:** a fictional `PV+ Demo Pharma` tenant and one demo login account exist in the Dev project per §48, kept out of the migration ledger (`supabase/seed.sql` for the tenant; the login account is Postgres-seeded — see README "Development environment"). Real case IDs follow the `PV-YYYY-NNNNNN` format.
- **This session's network policy blocks direct egress to external hosts generally**, confirmed against several: the Supabase project host, `api.vercel.com`, and `telemetry.vercel.com` all returned the identical `connect_rejected`/403 policy denial from the environment's egress proxy, whose own documentation says explicitly not to route around this and to report it instead. Server-side MCP tool calls (Supabase's `execute_sql`/`apply_migration`, GitHub's API) can reach their targets because they run outside this sandbox; a browser or CLI (`curl`, `vercel deploy`, Playwright) running inside this session cannot reach arbitrary external hosts. Concretely, this blocked: calling the Auth signup API directly (worked around by seeding the demo user via SQL instead), running the Playwright E2E suite to completion (validated the same flows via direct SQL instead), and **deploying to Vercel or any other hosting provider** — that one has no workaround; it needs either the environment's egress policy widened by an administrator, or a deploy run from a machine with normal network access. This is a constraint of the session that built this app, not a property of the app itself, and should not reproduce in a normal CI/deployment environment.
- **A real bug was caught, not just prevented, during Phase 1a:** the initial case-versioning trigger ran `BEFORE INSERT` and tried to write into `case_versions` referencing a `cases` row that did not exist yet, so every case creation failed with a foreign-key violation — undetected until a real SQL-level functional test was run (tsc/lint/build all passed regardless). Fixed in `0012_fix_case_version_trigger.sql`. Treat "builds and typechecks" as necessary, not sufficient — schema and RPC changes need to be exercised with real writes.
