...
- **This session's network policy blocks direct egress to the Supabase project host** (confirmed via the environment's proxy status — a `connect_rejected`/403 policy denial). Server-side MCP tool calls (`execute_sql`, `apply_migration`, etc.) can reach it; a browser or `curl` running inside this session cannot. This blocked calling the Auth signup API directly (worked around by seeding the demo user via SQL instead) and blocks running the Playwright E2E suite (`e2e/`) inside this session — it had to be validated by exercising the same SQL/RPCs directly instead. This is an environment constraint of the session that built this, not a property of the app; it should not reproduce in a normal deployment or CI environment with real network access.
